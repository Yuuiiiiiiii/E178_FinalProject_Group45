...
def unpack_gridsearch(gs):
    ...
    for f0val in f0vals:
        for f1val in f1vals:
            ...
            for col in cols:
                X[f0val].loc[f1val,col] = np.abs(float(result[col][ind][0]))

    return { 'scoregrid':X, 
             'best_params':gs.best_params_, 
             'best_estimator':gs.best_estimator_, 
             'best_score':np.abs(gs.best_score_),
...
def plot_grid_result(result,log=True):
    ...
    for f0ind, f0val in enumerate(f0vals):
        for c, col in enumerate(cols):
            score_data = np.abs(X[f0val][col])
            if log:
                plt.semilogx(score_data,color=colors[c],linestyle=linestyles[f0ind],marker='o',label='{} {}'.format(f0val,labels[c]))
            else:
                plt.plot(score_data,color=colors[c],linestyle=linestyles[f0ind],marker='o',label='{} {}'.format(f0val,labels[c]))
